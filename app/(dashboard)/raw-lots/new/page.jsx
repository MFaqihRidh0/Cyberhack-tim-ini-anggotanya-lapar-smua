...
export default function NewRawLotPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!form.supplierId || !form.materialId || !form.initialQty) {
      toast.error('Supplier, Material, dan Qty wajib diisi');
      return;
    }
    setLoading(true);
    try {
      await api.post('/raw-lots', form);
      toast.success('Raw Material Lot berhasil dibuat');
      router.push('/raw-lots');
    } catch (err) {
      toast.error(err.response?.data?.message || 'Gagal membuat lot');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-2xl font-bold text-slate-800 mb-6">Terima Raw Material Lot</h1>
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-xl border border-slate-200">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">No. Lot Supplier (opsional)</label>
          <input type="text" value={form.supplierLotNo} onChange={(e) => setForm({ ...form, supplierLotNo: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none" placeholder="Dari surat jalan" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Tanggal Expiry (opsional)</label>
