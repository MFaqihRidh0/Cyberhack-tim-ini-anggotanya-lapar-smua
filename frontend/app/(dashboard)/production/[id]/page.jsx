...
export default function ProductionDetailPage() {
  ...
  const [inputForm, setInputForm] = useState({ rawLotId: '', materialId: '', qtyUsed: '' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">{po.orderNumber}</h1>
          <p className="text-slate-500 mt-1">{po.product?.name} — Target: {formatNumber(po.targetQty)} {po.product?.unit}</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-3 flex-wrap">
        ...
        {po.actualQty && <span className="text-sm text-slate-600">Actual: {formatNumber(po.actualQty)} {po.product?.unit}</span>}
      </div>
        ...
        {addingInput && (
          <form onSubmit={handleAddInput} className="mb-4 p-4 bg-slate-50 rounded-lg space-y-3">
            <select value={inputForm.rawLotId} onChange={(e) => setInputForm({ ...inputForm, rawLotId: e.target.value })} className="w-full px-3 py-2 border border-slate-300 rounded-lg">
              ...
              {availableLots?.map((l) => <option key={l.id} value={l.id}>{l.internalLotNo} — {l.material?.name} ({formatNumber(l.initialQty)} {l.material?.unit})</option>)}
            </select>
          ...
          <tbody>
            {po.inputs?.map((inp) => (
              <tr key={inp.id} className="border-b border-slate-100">
                <td className="py-2 text-slate-800">{inp.rawLot?.internalLotNo}</td>
                <td className="py-2 text-slate-600">{inp.material?.name}</td>
                <td className="py-2 text-slate-600">{formatNumber(inp.qtyUsed)}</td>
                <td className="py-2 text-slate-500">{formatDate(inp.usedAt)}</td>
              </tr>
            ))}
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200">
          ...
          {po.finishedLots.map((fl) => (
            <div key={fl.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
              <span className="font-medium text-slate-800">{fl.lotNumber}</span>
              <span className="text-sm text-slate-600">{formatNumber(fl.quantity)} {fl.unit}</span>
              <StatusBadge status={fl.currentStatus} />
            </div>
          ))}
        </div>
      )}
